...
store_list = ['CD-Augusta']

for store in store_list:
    os.system('sed -i "s/CAFÉ/CAFE/g" ./linkcheck/'+store+'_liquor.csv')
    os.system('sed -i "s/\ -\ /-/g2" ./linkcheck/'+store+'_liquor.csv')
    os.system('sed -i "s/CHOPHOUSE\ -\ NOLA/CHOPHOUSE-NOLA/g" ./linkcheck/'+store+'_liquor.csv')
os.system('sed -i "s/CHOPHOUSE\ -\ NOLA/CHOPHOUSE-NOLA/g" ./linkcheck/liquor_ingredients.csv')
os.system('sed -i "s/CHOPHOUSE\ -\ NOLA/CHOPHOUSE-NOLA/g" ./linkcheck/liquor_vendor_items.csv')
os.system('sed -i "s/\ -\ /-/g2" ./linkcheck/liquor_ingredients.csv')
os.system('sed -i "s/\ -\ /-/g2" ./linkcheck/liquor_vendor_items.csv')
os.system('sed -i "s/CAFÉ/CAFE/g" ./linkcheck/liquor_ingredients.csv')
os.system('sed -i "s/CAFÉ/CAFE/g" ./linkcheck/liquor_vendor_items.csv')

os.system('clear')

for store in store_list:
    df_menuItems = pd.read_csv('./linkcheck/'+store+'_liquor.csv', sep=',')
    df_ingredients = pd.read_csv('./linkcheck/liquor_ingredients.csv', sep=',')
    #df_ingredients = takeoutgarbage(df_ingredients)
    df_stockCount = pd.read_csv('./linkcheck/'+store+'_Stock_Count.csv', sep=',')
    df_vendorItems = pd.read_csv('./linkcheck/liquor_vendor_items.csv', sep=',')
    ...
    with pd.ExcelWriter(f'./output/{store}_Liquor_Check.xlsx') as writer:     # pylint: disable=abstract-class-instantiated
        df_linkCheck.sort_values(by='POSI', inplace=True)
        df_linkCheck.to_excel(writer)
    print(df_linkCheck.info())

os.system("cp /home/wandored/Projects/r365cleaner/output/*_Liquor_Check.xlsx /home/wandored/Dropbox/Restaurant365/Report_Data")
